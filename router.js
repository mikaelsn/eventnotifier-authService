...
router.post('/sessions', function (req, res, next) {
  ...
  if (!params.username || !params.password || !params.deviceId) {
    return res.status(400).send({
      error: 'username, password, and deviceId ' +
        'are required parameters'
    });
  }
  ...
  User.findOne({ username: params.username }, function (err, result) {
    ...
    if (_.isNull(result)) {
      return res.status(401).send({ error: 'null: ' + result });
    }
    result.comparePassword(params.password).then(function (res) {
      if (!res) {
        return res.status(401).send({ error: 'compare: ' + res });
      }
    });
    return KeyService.set(result, params.deviceId).then(function (token) {
  ...
  // Promise.all([user, passwordMatch]).then(function(userResult, passwordMatchResult) {
    ...
    .then(function (result) {
      if (_.isNull(result)) {
        return res.status(404).send({
          error: 'Session does not exist or has ' +
